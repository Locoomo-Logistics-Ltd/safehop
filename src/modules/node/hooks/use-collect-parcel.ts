...
export function useCollectParcel(orderId: string) {
  ...
  const collectMutation = useMutation({
    mutationFn: ({ code, identityConfirmed }: { code: string; identityConfirmed: boolean }) =>
      nodeService.collectParcel(orderId, { code, identityConfirmed }),
    onSuccess: () => {
      setFailedCodeAttempts(0);
      // The parcel has left the building — refetch so it drops off the counter list.
  ...
  const resendMutation = useMutation({
    mutationFn: () => nodeService.resendCollectionCode(orderId),
    onSuccess: () => {
