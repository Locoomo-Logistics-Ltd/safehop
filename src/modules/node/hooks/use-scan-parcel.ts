 ...
 * preview.
 *
 * This used to call `nodeService.checkIn()` → the undocumented
 * `orders.scanHandoff` route, which resolved and checked the parcel in
 * atomically and needed a `qrNonce` plus the operator's live GPS. The
...
export function useScanParcel() {
  ...
  const openDropOff = useCallback(
    (rawScannedValueOrCode: string) => {
      ...
      router.push(ROUTES.nodeDropOff(encodeURIComponent(trackingCode)));
    },
    [router]
  );
