 ...
 * superseded `nodeService.checkIn()` did lookup+mutate atomically and
 ...
 *
 * The drop-off confirm used to hand-write the resolved order into a
 * device-local outgoing list (`store/node-outgoing.store.ts`) so the
 * rider-handoff screen would later have somewhere to find the uuid a
 * pickup code needs. That's no longer necessary: `GET
 * /handoffs/my-node/orders` (2026-08-17) now lists it server-side, so
 * this just invalidates that query and lets it refetch.
 */
export function useHandoffLookup(trackingCode: string) {
  const queryClient = useQueryClient();

  const lookupQuery = useQuery({
    queryKey: QUERY_KEYS.nodeHandoffOrder(trackingCode),
    queryFn: () => nodeService.lookupOrderByTrackingCode(trackingCode),
  ...
  const dropOffMutation = useMutation({
    mutationFn: (orderId: string) => nodeService.confirmDropOff(orderId),
    onSuccess: () => {
